...
# 가장 중요한건 확률변수, 확률변수는 함수, countT는 함수이자 확률변수
countT <- function(x){
  sum(x == "T")
}
...
# 확률 <- 값/표본공간의 수 
# table() category화, real number 각각의 수...
pX <- table(X) / nrow(S)
pX
plot(pX)

# 결론은 확률변수, 확률 질량 함수(PMF)의 이론
...
# ramdom variable 에 대한 PMF


## Self-Checking 3
xv <- 0:3
xp <- choose(3, 0:3)
# choose함수를 알려면 gamma?distribution을 알아야하지만 우린 지금 모른다. 고로 스킵
...
xcdf <- c(0, cumsum(xp))
sf <- stepfun(xv, xcdf)
# 확통가르치고 싶지 않으면 몰라도 되는 stepfun(), 중요한건 CDF CMF이론
...
#이론을 알면 코드가 간단한지 알 수 있다. 결론은 이론중요
